...
class Mp3ChipMH2024K16SS {
   public:
    ...
    static const SendPacket generatePacket(uint8_t command, uint16_t arg) {
        return {0x7E, 0xFF, 6, command, 0, static_cast<uint8_t>(arg >> 8), static_cast<uint8_t>(arg & 0x00ff), 0xEF};
    }
};
...
class Mp3ChipOriginal {
   public:
    ...
    static const SendPacket generatePacket(uint8_t command, uint16_t arg) {
        SendPacket packet = {
            0x7E, 0xFF, 6, command, 0, static_cast<uint8_t>(arg >> 8), static_cast<uint8_t>(arg & 0x00ff), 0, 0, 0xEF};
        setChecksum(&packet);
        return packet;
    }
};
...
class DFMiniMp3 {
   public:
    explicit DFMiniMp3(T_SERIAL_METHOD& serial, int8_t RX_Pin, int8_t TX_Pin)
        : _serial(serial), _lastSendSpace(c_msSendSpace), _isOnline(false), _RX_Pin(RX_Pin), _TX_Pin(TX_Pin) {}
    ...
    */
    ...
    void setPlaybackSource(DfMp3_PlaySource source) { sendPacket(0x09, source, 200); }
